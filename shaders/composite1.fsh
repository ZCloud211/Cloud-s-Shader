...
vec3 extractBloom(vec3 sceneColor, vec3 emissionRadiance) {
	...
	vec3 screenBloom = sceneColor * screenMask * 0.25;
	const float EMISSION_BLOOM_SCALE = 0.85;
	vec3 emissiveBloom = emissionRadiance * EMISSION_BLOOM_SCALE;
...
vec3 sampleExtractedBloom(vec2 sampleUV) {
	...
	vec3 emissionRadiance = texture(colortex3, sampleUV).rgb;
	return extractBloom(sceneSample, emissionRadiance);
}
